...
# 
# USO:
#   flatpak-cache-maintenance.sh              # Execução normal (respeita flag diária)
...
main() {
    ...
    if ! flatpak_maint_ensure_nfs; then
        flatpak_maint_log "❌ NFS não disponível. Abortando."
        exit 1
    fi

    # 2. Verifica repositório
    if [ ! -d "$FLATPAK_MAINT_REPO_PATH" ]; then
        flatpak_maint_log "❌ Repositório não encontrado: $FLATPAK_MAINT_REPO_PATH"
        exit 1
    fi
    ...
    if [ "$FORCE_EXECUTION" = false ] && [ "$UPDATE_FLAG" = false ]; then
        if ! flatpak_maint_is_due "$FLATPAK_MAINT_FLAG_FILE"; then
            flatpak_maint_log "📅 Manutenção já executada hoje. Use --force para forçar."
            exit 0
        fi
    fi
    ...
    if ! flatpak_maint_acquire_lock "$FLATPAK_MAINT_LOCK_DIR"; then
        flatpak_maint_log "❌ Não foi possível adquirir lock. Abortando."
        exit 1
    fi

    # 5. EXECUTA OSTREE FSCK, REPAIR E PRUNE (PONTO ÚNICO DE EXECUÇÃO)
    ...
    if [ $exit_code -eq 0 ] || [ "$UPDATE_FLAG" = true ]; then
        ...
        flatpak_maint_log "⚠️ Marcador NÃO atualizado devido a erros (código $exit_code)"
    fi
